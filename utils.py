...
def calc_risk_metrics(data, as_df=False):
    """
...
def print_max_min_correlation(corrs):
    ...
    max_corr_raw = corr_series.agg(["idxmax", "max"]).T
    ...
    max_corr_raw, max_corr_raw_val = max_corr_raw["idxmax"], max_corr_raw["max"]
    ...
    print(
        f"Max Corr (raw): {max_corr_raw[0]} and {max_corr_raw[1]} with a correlation of {max_corr_val:.2f}"
    )
    print(
        f"Min Corr (raw): {max_corr_raw[0]} and {max_corr_raw[1]} with a correlation of {min_corr_raw_val:.2f}"
    )
